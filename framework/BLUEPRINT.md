...
- **2026-08 · §11 — three-level Just the Docs nav (`parent`/`grand_parent`/`has_children`) 404'd live; flattened to match the proven pattern.** First attempt nested the four new pages under an intermediate `framework-reference.md` hub. After pushing, `betat.org/framework-cli.html` 404'd — root cause not fully chased down (likely a Just the Docs version mismatch in the `remote_theme` resolution), but the fix was straightforward: flatten all four to `parent: For Builders` directly, `nav_order` 5–8, the exact pattern ARCHITECTURE.md/COMMUNITY_FRAMEWORK.md/PROVENANCE_SPEC.md/RENDERING.md already use successfully on this site. Lesson for future doc pages on this site: don't reach for multi-level nav features without confirming they're supported first — match what's already proven to work.
