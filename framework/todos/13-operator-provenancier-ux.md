...
- [x] **Passphrase visibility toggle — REVERSED 2026-09-14 (was: DECIDED 2026-09-13, no JS in v0.1).** The 2026-09-13 decision below was correct about the technical constraint (CSS cannot mutate an `<input>`'s `type` attribute or read its live value — no CSS-only middle option exists) but was reopened and reversed on explicit developer direction the next session: the bundled UI now ships its first JavaScript, `bundledui/static/bundledui/scripts/betat.js`, a small progressive-enhancement script that wraps every `input[type="password"]` on the page and adds a `Show`/`Hide` toggle button, loaded once site-wide via `base.html`. No template changes were needed per password field — the script targets input type, not specific ids, so `enroll.html`, `provenancier_login.html`, `claim.html`, and `rotate_passphrase.html` all picked it up automatically. Every form still works with JS disabled; the double-entry confirm fields described below remain as a second, independent mitigation. Full detail: BLUEPRINT §07's 2026-09-14 Decision Log entry. **Original 2026-09-13 reasoning, preserved for context:** Developer's call, resolving the open JS-vs-no-JS question above. A real reveal-on-click toggle is impossible in pure CSS regardless of the checkbox-hack technique — CSS cannot mutate an `<input>`'s `type` attribute, and there is no CSS-only way to mirror a password field's live value into a visible decoy field either (that mirroring itself requires JS to read the input event). So "CSS-only decoy field" was never actually a viable middle option once examined closely — the real choice was "add the first functional JS in the community-facing UI" vs. "ship nothing here." Decided at the time: nothing, for v0.1. **What already covers the underlying risk:** `enroll.html`'s existing `passphrase`/`passphrase_confirm` double-entry (`forms.py` `EnrollForm`) catches the most common failure mode — typing two *different* things — with a clear "Passphrases do not match" error before submission. It does not catch typing the *same* wrong thing twice (a consistent typo, or simply forgetting the passphrase immediately after choosing it), which a visual reveal would have caught; that residual gap is now also covered by the toggle. `provenancier_login.html`'s single passphrase field also picked up the toggle automatically, though it needs no matching double-entry — a mistyped *login* passphrase fails cleanly (`invalid_credentials`) and is safely retryable, unlike a mistyped *enroll* passphrase, which could create an identity keyed to a passphrase the applicant doesn't actually know.
...
- [x] Passphrase fields let a user verify what they typed before submitting a value that can never be recovered if mistyped — met by `enroll.html`'s existing `passphrase`/`passphrase_confirm` double-entry (catches mismatched typos) **and**, as of 2026-09-14, a real Show/Hide visibility toggle (see Tasks above — reverses the earlier no-JS decision). `provenancier_login.html` needs no double-entry — a mistyped login passphrase is safely retryable, not unrecoverable — but picked up the visibility toggle automatically too.
...
- Any real JS added for the passphrase toggle must not log, store, or transmit the passphrase anywhere it doesn't already go — **addressed 2026-09-14**: `betat.js`'s toggle only flips the input's `type` attribute client-side; it never reads, copies, logs, or transmits the value anywhere. See Tasks above for the reversal of the earlier no-JS decision.
...
## Addendum — 2026-09-14, post-ship follow-ups found during manual walkthrough

Two items surfaced testing the shipped v0.1, both resolved same session (full detail in BLUEPRINT
§03/§07's 2026-09-14 Decision Log entries — summarized here since this file is read first for this area):

1. **Enroll page's `vouchers` field was dead code.** `EnrollForm.applicant_payload()` still packaged a
   typed comma-separated `vouchers` list, but `PeerVouchAuth.enroll()` has ignored it since the
   2026-09 two-phase-vouch refactor — typing names into it did nothing. Removed; replaced with a
   read-only member list on the enroll page (new `GET /betat/provenanciers`, public, identity +
   display_name only), so an applicant can see real members to ask to vouch, without exposing anything
   the picker submits — vouching still only happens via the voucher's own authenticated
   `POST /betat/vouch/{request_id}`. Privacy tradeoff (member roster becomes directly enumerable,
   though already recoverable via record-crawling) considered and accepted — see the Decision Log
   entry for full reasoning.
2. **Passphrase visibility toggle reversed** — see the Tasks section above, this file's own task 4
   entry now reflects the 2026-09-14 reversal in place rather than as a separate note here.
