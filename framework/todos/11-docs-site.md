...
- **New pages nest under the existing "For Builders" section** using Just the Docs' three-level nav (`parent`/`grand_parent`/`has_children`), matching the exact pattern ARCHITECTURE.md/COMMUNITY_FRAMEWORK.md/PROVENANCE_SPEC.md/RENDERING.md already use: `framework-reference.md` (nav_order 5 under For Builders, `has_children: true`) → `framework-cli.md`, `framework-api.md`, `framework-store.md` (its children).
...
1. Confirm the Jekyll site builds cleanly with the new pages (GitHub Pages build, or `bundle exec jekyll build` locally if you have Ruby set up) — nav placement and permalinks weren't verified against a live build in this session, only against `_config.yml` and existing pages' front matter.
2. `pytest tests/` (full suite, including the updated `test_bundledui.py`)
3. Once both check out: flip `TODO.md`'s status to `done`
