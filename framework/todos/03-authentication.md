...
### Addendum (§12 session, 2026-09-09) — peer-vouch bootstrap catch-22, fixed post-close
Surfaced by real dist-package install testing (TODO 12): an operator who enables only `community_peer_vouching` during `betat init` gets a community with no possible first member — 2 vouches can never arrive when 0 people are enrolled. The §10 Decision Log's original answer (route a fresh community's first members through `CryptoKeyAuth` instead) only helps operators who also happen to enable that method; nothing in `betat init` warns against enabling peer-vouch alone. Fixed with a **founding phase**: `PeerVouchRequest` gained a `founding` `BooleanField`; `PeerVouchAuth.enroll()` now checks `Provenancier.objects.count() < 2` at request creation and, if true, routes to a new `pending_admin` outcome instead of the vouch-threshold path — promoted via a new Django admin action (`communityauth/admin.py`'s `approve_founding_requests`), not member vouches. `bundledui/views.py`/`enroll_pending.html` gained a matching third UI state (no vouch language, "Awaiting administrator approval"). Full rationale, superseding the §10 decision: BLUEPRINT §03 Decision Log, 2026-09-09. Four existing tests in `test_communityauth.py`/`test_bundledui.py` were updated (they encoded the old zero-enrolled-members-uses-vouch-threshold assumption) and five new tests added, covering both the founding path and the admin action. Doesn't change this section's status. **Developer action needed:** `python manage.py makemigrations communityauth && python manage.py migrate` (new field, not yet migrated) before `pytest tests/` will pass — the new tests create `PeerVouchRequest` rows and need the `founding` column to exist.
